...
echo "PSQL::4=${PSQL::4}"
echo "REALM=${REALM:?environment variable REALM is not set}"
...
echo "min-port=${COTURN_MIN_PORT:-49152}" >> /etc/turnserver.conf
echo "max-port=${COTURN_MAX_PORT:-51609}" >> /etc/turnserver.conf
...
echo "cert=${HTTPS_CERT_FULLCHAIN:-/certs/cert.pem}" >> /etc/turnserver.conf
echo "pkey=${HTTPS_CERT_PRIVKEY:-/certs/key.pem}" >> /etc/turnserver.conf
